...
for STUDENT_MODEL_CKPT in "${STUDENT_MODEL_CKPT_LIST[@]}"; do
    if [ "${DISTILL_TYPE}" == "base" ]; then
        ...
        echo ""
        echo "Eval student model ${STUDENT_MODEL_NAME} (${STUDENT_MODEL_CKPT} on ${DISTILL_DATASET_NAME}) distilled from reason pretrained SAE at teacher model ${TEACHER_MODEL_NAME} (${TEACHER_MODEL_CKPT}) post-trained on ${PT_DATASET_NAME}"
        echo ""
    fi

    python ./scripts/eval/run_post_train_merge_distill_reason_pretrained.py \
    ...
    if [ "${DISTILL_TYPE}" == "base" ]; then
        MODEL_PATH="${CKPT_DIR}/models/${STUDENT_MODEL_NAME}/base/distill/${DISTILL_DATASET_NAME}/pretrain/${DISTILL_SAE_HOOKPOINT}/${STUDENT_MODEL_CKPT}-merged"
    else
        MODEL_PATH="${CKPT_DIR}/models/${TEACHER_MODEL_NAME}/${PT_TYPE}_${PT_DATASET_NAME}/${TEACHER_MODEL_CKPT}/distill/${DISTILL_DATASET_NAME}/pretrain/${DISTILL_SAE_HOOKPOINT}/${DISTILL_TYPE}/${STUDENT_MODEL_CKPT}-merged"
    fi
    ...
    for TASK in "${tasks[@]}"; do
        ...
        lighteval vllm $MODEL_ARGS "custom|$TASK|0|0" \
            ...
            --use-chat-template \
            --output-dir "${OUTPUT_DIR}/${TASK}/${STUDENT_MODEL_NAME}" # TODO
    done
done
