...
#DATASET_NAME="curated_openthoughts3"

START_LAYER=12
END_LAYER=12
STEP=4

PY_SCRIPT="./scripts/train/run_pretrain_sae.py"
PY_CONFIG="./recipes/${MODEL_NAME}/${POST_TRAIN_TYPE}/pretrain_sae_${DATASET_NAME}.yaml"
...
for ((i=START_LAYER; i<=END_LAYER; i+=STEP)); do
    ...
    torchrun --nproc_per_node="${GPU_COUNT}" --master-port=29501 "${PY_SCRIPT}" "${ARGS[@]}"
done
